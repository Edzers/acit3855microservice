...
export default function EndpointAudit(props) {
    ...
    const getAudit = () => {
        fetch(`http://acit3855group45kafka.eastus2.cloudapp.azure.com:8110/${props.endpoint}?index=${rand_val}`)
            .then(res => res.json())
            .then((result) => {
                console.log("Received Audit Results for " + props.endpoint)
                setLog(result);
                setIndex(rand_val); // Set the index here to ensure it is in sync with the data
                setIsLoaded(true);
            }, (error) => {
                setError(error);
                setIsLoaded(true);
