...
def get_rate_seller_event(index):
    """Get rate_seller event in History."""
    hostname = f"{app_config['events']['hostname']}:{app_config['events']['port']}"
    client = KafkaClient(hosts=hostname)
    topic = client.topics[str.encode(app_config["events"]["topic"])]
    ...
    try:
        ...
        for msg in consumer:
            msg_str = msg.value.decode('utf-8')
            msg = json.loads(msg_str)
            if msg.get('event_type') == 'rate_seller' and current_index == index:
                # Found the event at the index you want
                # return the event and a 200 status code
                return msg, 200
            if msg.get('event_type') == 'rate_seller':
                # Increment the index for rate_seller events only
                current_index += 1
    except Exception as e:
        logger.error("No more messages found or an error occurred: %s", str(e))
